...
from models.business import (Business, BusinessCreate, BusinessResponse,
                             BusinessUpdate)
from models.company_profile import (CompanyProfile, CompanyProfileCreate,
...
async def create_business(business_data: BusinessCreate, db: Session = Depends(get_db)):
    """Create a new business entity"""
    service = BusinessService(db)
    business = service.create_business(
        user_id=business_data.user_id, business_data=business_data.dict()
    )
    return business
...
async def get_user_businesses(user_id: int, db: Session = Depends(get_db)):
...
async def update_business(
    business_id: int, update_data: BusinessUpdate, db: Session = Depends(get_db)
):
    """Update business information"""
    service = BusinessService(db)
    business = service.update_business(
        business_id, update_data.dict(exclude_unset=True)
    )
    if not business:
        raise HTTPException(status_code=404, detail="Business not found")
    return business
...
async def verify_business(
    business_id: int, verification_data: dict, db: Session = Depends(get_db)
):
...
async def create_company_profile(
    business_id: int, profile_data: CompanyProfileCreate, db: Session = Depends(get_db)
):
...
async def get_company_profile(business_id: int, db: Session = Depends(get_db)):
...
async def update_company_profile(
    business_id: int, update_data: CompanyProfileUpdate, db: Session = Depends(get_db)
):
