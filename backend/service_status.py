...
def display_status_table(statuses: List[Dict]):
    ...
    print(f"{backend['service']:<25} {backend['status']:<15} {backend['port']:<8} {details:<30}")

    # Service statuses
    for status in statuses[1:]:
        details = status.get('error', f"Response: {status.get('response_time', 'N/A')}s")
        print(f"{status['service']:<25} {status['status']:<15} {status['port']:<8} {details:<30}")
