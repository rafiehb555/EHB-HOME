...
class IntegrationTester:
    ...
    async def test_google_services(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"✅ Google Cloud Services: {sum(results.values())}/{len(results)} passed")
    ...
    async def test_payment_services(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if Config.STRIPE_SECRET_KEY:
                stripe_result = await service_manager.payments.create_stripe_payment_intent(
                    1000,  # $10.00
                    'usd'
                )
    ...
    async def test_communication_services(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if Config.TELEGRAM_BOT_TOKEN:
                telegram_result = await service_manager.communication.send_telegram_message(
                    "123456789",
                    "Hello from EHB!"
                )
                results['telegram_message'] = telegram_result['success']

            print(f"✅ Communication Services: {sum(results.values())}/{len(results)} passed")
    ...
    def generate_report(self) -> str:
        ...
        for service_name, service_results in self.results.items():
            ...
            status = "✅ PASS" if service_passed == service_total else "⚠️  PARTIAL" if service_passed > 0 else "❌ FAIL"
        ...
        # Overall summary
        overall_status = "✅ ALL PASSED" if passed_tests == total_tests else "⚠️  PARTIAL" if passed_tests > 0 else "❌ ALL FAILED"
        report_lines.append("=" * 50)
