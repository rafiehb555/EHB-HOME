...
async def get_service_profile(service_name: str, user_id: int, current_user: User = Depends(get_current_user)):
...
async def verify_with_service(service_name: str, verification_data: dict, current_user: User = Depends(get_current_user)):
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            if service_name == "pss":
                response = await client.post(f"{service_config['url']}/verify", json=verification_data)
            elif service_name == "emo":
                response = await client.post(f"{service_config['url']}/verify-business", json=verification_data)
            elif service_name == "edr":
                response = await client.post(f"{service_config['url']}/register-exam", json=verification_data)
            else:
                raise HTTPException(status_code=400, detail="Service does not support verification")
...
async def get_user_verification_status(user_id: int, current_user: User = Depends(get_current_user)):
    ...
    verified_services = sum(1 for status in verification_status.values() if status["status"] == "verified")
    total_services = len(verification_status)
    progress_percentage = (verified_services / total_services) * 100 if total_services > 0 else 0
